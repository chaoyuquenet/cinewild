...
export const addMovie = (title, type, language) => async dispatch => {
  const movieAddResult = await axios.post(`${ROOT_URL}/movies`, {
    title,
    type,
    language
  });
...
const getPosters = async localResults => {
  const theMovieDbResults = await Promise.all(
    localResults.data.map(
      async movie =>
        await axios.get(
          `https://api.themoviedb.org/3/search/movie?api_key=0dc87e3b02d544111b18d319a319ef56&&query=` +
...
export const searchMovieByProperty = (
  property,
  value,
  target
) => async dispatch => {
  const localApiResult = await axios.post(`${ROOT_URL}/movies/search`, {
    [property]: value
  });
